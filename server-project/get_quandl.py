...
def codes_per_exchange():
    ...
    codes = pd.read_csv("codes.csv", header=0, names=["code","name"]) 
...
# write all column headers to a txt file for later usage (did this only once)
#all_columns = []
#for exchange, list_of_codes in codes_per_xch.items():
#    # we only need one code per exchange in order to see its column headers
#    # assumes that the data delivered per exchange is identical
#    print("\n")
